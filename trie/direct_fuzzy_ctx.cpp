...
struct direct_fuzzy : Trie {
	...
	unsigned dist = 0;
	while(char c = *str++)
	    if((dist += cost.del(c,*str)) > limit) break;
	return dist;
    }
...
//printf("%*c", ofs*4, ' '), printf("[%c<->%c]\n",  ct, c),
		    if(Trie* p = entry.find_node(tmp, zero, 0))
		    	enqueue(static_cast<direct_fuzzy*>(p), inp+1, ofs+2, ndist+penalty);
		}
#endif
		ndist += cost.del(ct,*inp);
		if(ndist > limit)
		    return;
//printf("%*c", ofs*4, ' '), printf("[del %c]\n", ct);
	    } 
	    if(ndist+(penalty=cost.ins(c,0)) <= limit) 
		enqueue(trie, inp-1, ofs+1, ndist+penalty);
	}
